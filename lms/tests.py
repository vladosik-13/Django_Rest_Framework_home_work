...
class LessonCRUDTests(TestCase):
    def setUp(self):
        ...
        self.moderator_user = User.objects.create_user(email='moderator@example.com', password='moderatorpassword')
        self.regular_user = User.objects.create_user(email='user@example.com', password='userpassword')
        self.another_user = User.objects.create_user(email='anotheruser@example.com', password='anotherpassword')
        ...
        self.lesson = Lesson.objects.create(course=self.course, title='Test Lesson', description='Test Lesson Description', owner=self.admin_user)
    ...
    def test_delete_lesson_as_admin(self):
        self.client.login(email='admin@example.com', password='adminpassword')
        url = reverse('lesson_delete', kwargs={'pk': self.lesson.id})
    ...
    def test_retrieve_lesson(self):
        self.client.login(email='user@example.com', password='userpassword')
        url = reverse('lesson_retrieve', kwargs={'pk': self.lesson.id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
